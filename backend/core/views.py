...
from .models import User, Account, Category, Service, Movement, ExchangeRate, AccountLimit
from .serializers import UserSerializer, AccountSerializer, CategorySerializer, ServiceSerializer, MovementSerializer, ExchangeRateSerializer, AccountLimitSerializer
...
class MovementViewSet(viewsets.ModelViewSet):
    queryset = Movement.objects.select_related('id_account', 'id_destination_account', 'id_category', 'id_service', 'id_exchange_rate').all()
    ...
    def create(self, request, *args, **kwargs):
        ...
        if destination_id:
            ...
            if missing:
                return Response({'error': 'Faltan campos obligatorios', 'fields': missing}, status=status.HTTP_400_BAD_REQUEST)
            ...
            with transaction.atomic():
                last_group = Movement.objects.order_by('-transfer_group_id').values_list('transfer_group_id', flat=True).first() or 0
            ...
            return Response({'message': 'Transferencia creada', 'salida': MovementSerializer(salida).data, 'entrada': MovementSerializer(entrada).data}, status=status.HTTP_201_CREATED)
